...
export type UserCallback = (err: Error | { Error: string } | null, user?: IUser | IUser[] | null) => void;
...
export function createUser(requestbody: Partial<IUser>, callback: UserCallback): void {
    ...
    query.exec(function (err: Error | null, user: IUser | null) {
        if (user) {
            ...
            User.create(requestbody, function (err: Error | null, user: IUser | null) {
                if (err) {
...
export function updateUser(searchUserID: { userID: string }, requestbody: Partial<IUser>, callback: UserCallback): void {
    ...
    query.exec(async function (err: Error | null, user: IUser | null) {
        if (user) {
            ...
            return callback({ Error: "User for userID: " + searchUserID.userID + " not found" });
        }
    });
}

// Löscht einen User aus der Datenbank
export function deleteUser(userID: { userID: string }, callback: UserCallback): void {
        ...
        query.exec(function (err: Error | null, user: IUser | null) {
            if (err) {
                ...
                if (user) {
                    ...
                    callback({ Error: "UserService: Could not find user for user ID: " + searchUserID }, null);
                }
            }
        });
    }
